...
function getUtentiAttivi(listaUtenti) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      let utentiAttivi = listaUtenti.filter(utente => utente.attivo);
      if (utentiAttivi.length > 0) {
        resolve(utentiAttivi);
      } else {
        reject("Non ci sono utenti attivi");
      }
    }, 2000);
  });
}
...
// getUtentiAttivi(utenti)
//   .then(result => {
//     console.log(result);
//     console.log(`Numero di utenti attivi: ${result.length}`);
//   })
...
// La funzione deve:
// - cercare nell’array prodotti il prodotto con l’id passato;
...
function trovaProdotto(idProdotto) {
  return new Promise((resolve, reject) => {
    const prodotto = prodotti.find(prodotto => prodotto.id === idProdotto);
    if (prodotto) {
      resolve(prodotto)
    }
    else {
...
// - usa await per ottenere l’array studenti;
...
async function preparaReportStudenti() {
  try {
    let risultato = await recuperaDati()
    let trasformato = risultato.map(({ nome, voto }) => `${nome} - voto ${voto}`);
    let maggiore = risultato.filter((studente) => studente.voto >= 28).length
    return { trasformato, maggiore }
...
// preparaReportStudenti()
// .then(({trasformato,maggiore})=> console.log(trasformato,maggiore))
// .catch((error)=> console.error(error))

// Esercizio 4
// Crea una funzione fetchOrdini() che restituisca una Promise e risolva con l’array ordini.
...
async function analizzaOrdini() {
  try {
    let ordinii = await fetchOrdini()
    let ordflt = ordinii.filter((ordine) => ordine.pagato === true);
    let tot = ordinii.reduce((acc, cur) => acc + cur.totale, 0);
    let clnt = ordinii
      .filter(({ pagato }) => pagato)
      .map(({ cliente }) => cliente);
    return { ordflt, tot, clnt }
...
function formattaLibri(listaLibri) {
  return new Promise((resolve, reject)=>{
    let val=true;
    let libriFilter= listaLibri.filter(({disponibile})=> disponibile === true);
  let arrNew= libriFilter.map(({ titolo, autore }) => ({ titolo, autoreCompleto: autore }))
    if(val){
      resolve(arrNew)
    } else { reject(error)}
})}
//  getLibri()
//   .then(libri => formattaLibri(libri))  
...
function getUtentiConProfili(){
  return new Promise((resolve,reject)=>{
let valo=true;
if(valo){
  resolve(users)
}else {reject("erroreee")}
  })
...
async function stampaProfili() {try {
   let aray= await getUtentiConProfili()
  let strng= aray.map(({nome, profilo :{email, ruolo}})=>`${nome}|${email}|${ruolo}`)
 let flt= aray.filter(({profilo:{ruolo}}) => (ruolo !== "user"))
...
function getDettagliCarrello(carrello, catalogo){
return new Promise ((resolve, reject)=>{
  let esiste=10;
})}
